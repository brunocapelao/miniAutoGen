...
| **Runtime próprio** | Não (só protocol) | Sim (loop + tools) | Não (só protocol) | Mínimo (loop simples) | Sim (loop + tools) | ✅ Agent Runtime (proposto) |
| **Tool calling** | Via provider | Nativo + guardrails | Via provider | Nativo | Nativo + MCP | ✅ ToolProtocol + MCP |
| **Observabilidade** | Não | Básica | Não | Não | Tracing built-in | ✅ 47+ EventTypes |
...
| Observabilidade | Depende do provider | EventSink com 47+ event types |
...
| **Observabilidade built-in** | 47+ event types canônicos; qualquer Engine é observável |
